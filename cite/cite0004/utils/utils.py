...
## Preprocess
def gex_preprocess(cfg, rna_df):
    rna = AnnData(rna_df)
    sc.pp.filter_cells(rna, min_genes=200)
    rna.var['mt'] = rna.var_names.str.contains('_MT-')
    rna.X.data = np.exp(rna.X.data) - 1.
    rna.obs['percent_mito'] = np.sum(rna[:, rna.var['mt']].X, axis=1).A1 / np.sum(rna.X, axis=1).A1
    rna = rna[rna.obs['percent_mito'] < 0.25, :]
    sc.pp.log1p(rna)
    sc.pp.highly_variable_genes(rna, min_mean=0.05, max_mean=7, min_disp=0.5)
...
def load_data(cfg, data_dir, compressed_data_dir):
    ...
    if cfg.pca_input:
        ...
        gc.collect()
        ## 最大値で割って0-1に正規化
        max_input = torch.from_numpy(np.load(data_dir / 'train_cite_inputs_max_values.npz')['max_input'])[0].to(cfg.device)
    ...
    # 訓練データのターゲットの読み込み
    compressed_target_path = compressed_data_dir / f'train_cite_target_tsvd{cfg.latent_target_dim}_seed{cfg.seed}.pkl'
    compressed_target_model_path = compressed_data_dir / f'train_cite_target_tsvd{cfg.latent_target_dim}_seed{cfg.seed}_model.pkl'
